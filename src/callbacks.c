...
void nearby_menu_draw_row_callback(GContext* ctx, const Layer *cell_layer, MenuIndex *cell_index, void *data) {
  ...
  if (cell_index->section == 0) {
		...
		if(!nearby_buses[arr_pos].bus_route){
		...
		//draw the row
    menu_cell_basic_draw(ctx, cell_layer, nearby_buses[arr_pos].title, nearby_buses[arr_pos].stop, NULL);
	...
	if(cell_index->section == 0) {
		switch(cell_index->row){
			case 0:
				...
				//then push the nearby buses window to show those buses
...
void nearby_menu_select_callback(MenuLayer *menu_layer, MenuIndex *cell_index, void *data) {
  // Use the row to specify which item will receive the select action
	...
	//don't do anything if we haven't gotten all buses
	if(num_nearby_buses==0){
		return;
	}
	
	selected_bus_row = menu_layer_get_selected_index(nearby_menu_layer);	
