...
void nearby_buses_window_load(Window *window) {
	...
	//tell javascript to get updated bus info
	
	//moved this to the menu select to stop reloading on back button
	//get_buses();
	...
	text_layer_set_text(loading_text_layer, "Loading Buses, please wait...");
	
	//needed so it isn't leaking memory from first init
	menu_layer_destroy(nearby_menu_layer);
	nearby_menu_layer = menu_layer_create(bounds);


	//set callbacks for the nearby buses menu
  ...
  layer_add_child(nearby_buses_window_layer, menu_layer_get_layer(nearby_menu_layer));
	...
	layer_add_child(nearby_buses_window_layer, text_layer_get_layer(loading_text_layer));
...
void bus_info_window_unload(){
	...
	menu_layer_set_selected_index(nearby_menu_layer, selected_bus_row, MenuRowAlignCenter, false);
...
void free_bus_info(int num_buses){
	int i;
